...
public class FingerUtil {
    ...
    public FingerUtil(Activity parentActivity, Handler handler) {
        if (m_devComm == null) {
    ...
    public FingerUtil(Activity parentActivity, FingerprintPresenter fingerprintPresenter) {
        if (m_devComm == null) {
            m_devComm = new DevComm(parentActivity, m_IConnectionHandler);
        }
    ...
    public void StartSendThread() {
        new Thread(new Runnable() {
            public void run() {
                ...
                if (w_blRet == false) {
                    ELog.i("==========Fail to receive response! ======== Please check the connection to target.============");
                    return;
                }
                ...
                if (w_nRet == (short) DevComm.ERR_SUCCESS) {
                    switch (w_nData) {
                        ...
                        case (short) DevComm.GD_NEED_THIRD_SWEEP:
                            ...
                            break;
                        default:
                            ELog.i("=====识别====录入指纹=====成功==id====" + w_nData);
                            if (WiscClient.isEnter == true) {
                                mFingerprintPresenter.showMessage(1, 0);
                            } else {
                                mFingerprintPresenter.showMessage(2, w_nData);
                            }
                            break;
                    }
                } else {
